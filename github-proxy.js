...
// ── LIMITI DIMENSIONE ──
const MAX_CONTENT_B64 = 100 * 1024 * 1024; // ~75 MB file + overhead base64 (~33%)
const MAX_MESSAGE_LEN = 200;
const MAX_BODY_BYTES  = 105 * 1024 * 1024; // 105 MB body totale
...
exports.handler = async (event) => {
  ...
  if (action === 'put') {
    ...
    if (content.length > MAX_CONTENT_B64)
      return errResp(413, 'Contenuto troppo grande (max 75 MB).', origin);
    if (!/^[A-Za-z0-9+/\n]+=*$/.test(content))
